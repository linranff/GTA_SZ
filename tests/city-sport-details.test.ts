...
test('actual GT mounts verify front, rear, original body surface and four exhaust outlets within the original footprint',async()=>{
 ...
 for(const support of mount.supports){assert(support.z< -2);assert(support.y>1&&support.y<1.15);assert.equal(support.y,mount.topAt(support.x,support.z));assert(mount.wing.height-support.y>.2&&mount.wing.height-support.y<.4);}
...
test('sport details follow a moving transformed car, keep materials isolated, and stay under four meshes and 2500 triangles',async()=>{
 ...
 const result=createCitySportDetails(scene,car);assert.equal(result.stats.applied,true,result.stats.skippedReason??'sport kit should apply');assert.equal(result.stats.meshCount,4);assert(result.stats.triangles<2500);assert.equal(scene.meshes.length,count+4);assert.equal(scene.lights.length,0);assert.notEqual(head.material,original);
 ...
 for(const mesh of result.meshes){assert.equal(mesh.parent,car);assert.equal(mesh.isPickable,false);assert.equal(mesh.checkCollisions,false);const b=mesh.getBoundingInfo().boundingBox;assert(b.minimum.x>=-1.027&&b.maximum.x<=1.027);assert(b.minimum.z>=-2.501&&b.maximum.z<=2.501);assert(b.maximum.y<1.5);}
