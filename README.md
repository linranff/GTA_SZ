...
| Lawns and parks look bare from the air | A separate 10–30 m canopy layer: six solid-crown species (giant banyan, kapok, camphor, terminalia, flame tree, mango), 45,626 trees placed in parks, open lawns and along trunk/primary roads with offline clearance proofs; three LODs, distance-demoted budgets; ground blades halved | Costs about 1–5 fps, mostly shadow passes; art-directed species, not a vegetation survey. [Record](docs/graphics/canopy-trees-2026-09-17.md) |
| Junctions had no signals; streets had no e-bikes | Cantilever traffic lights at 395 arterial junctions clustered from shared OSM way nodes (stop line past the crossing carriageways, pole on the driver's right or the median), two-group fixed cycle; NPC cars, riders and the autopilot hold at red. 21,110 e-mopeds/e-bicycles/courier scooters in kerbside rows outside metro exits, bus stops and dense streets, tinted per instance; a small pool of helmeted riders follows the kerb | Signal positions and parking rows are game adaptations, not surveyed; riders do not yet yield to cars or pedestrians. [Signals](src/city-traffic-signals.ts) · [E-bikes](src/city-ebikes.ts) |
