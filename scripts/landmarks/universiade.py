...
def _crystal(b, scale, item, material):
    ...
    for i in range(6):
        ...
        b.tube("steel", (x0, y0, h * 0.12), (x1, y1, h * 0.96), 0.35 * scale, 5)
    return h
...
def build(b, lm, spec, scale=0.6):
    ...
    b.box("water", (lx, ly + float(lake["ry_m"]) * 0.95 * scale, 3.6 * scale), (float(lake["rx_m"]) * 1.4 * scale, 16 * scale, 7.2 * scale))
    # South terrace so the front camera sees water, not only the north lake.
    b.box(
        "water",
        (0, -float(spec["stadium"]["ry_m"]) * 1.08 * scale, 4.8 * scale),
        (float(spec["stadium"]["rx_m"]) * 1.55 * scale, 26 * scale, 9.6 * scale),
    )
