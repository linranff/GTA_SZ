...
const duration=Number(process.env.DRIVE_SECONDS??300),errors=[],timeline=[],arrivals=[],resources=[];
...
let final,frames=[],start,initial,elapsed=0,hashes=[],gpuDiagnostics=null;
...
try{
 await page.goto('http://127.0.0.1:4173/',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>window.__SHENCHENGJI_CITY__?.ready,null,{timeout:120000});await page.waitForTimeout(2200);
 await page.keyboard.press('l');await page.waitForTimeout(1000);initial=await page.evaluate(()=>window.__SHENCHENGJI_CITY__.stats);await page.screenshot({path:path.join(out,'01-night-start.png')});
