...
const detail=JSON.parse(await fs.readFile(p+'landmark-detail.json','utf8')),candidates=inventory['landmark-candidates.json']?JSON.parse(await fs.readFile(p+'landmark-candidates.json','utf8')):null,excludedIds=[...new Set([...detail.baseBuildingIds,...(candidates?.baseBuildingIds??[])])].sort(),assetFiles=Object.keys(assets),totalGlbBytes=Object.values(assets).reduce((n,a)=>n+a.bytes,0);
...
const writes={
 'building-exclusions.json':{excludedIds,assets:{'buildings.glb':assets['buildings.glb'],'facades.glb':assets['facades.glb']}},
