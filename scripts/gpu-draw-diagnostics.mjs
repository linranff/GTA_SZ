...
export function installGpuDrawDiagnostics(){
 ...
 const sources=new WeakMap(),attached=new WeakMap(),ids=new WeakMap(),fbMasks=new WeakMap(),seen=new Set();let nextId=1,program=null,framebuffer=null,backMask=[1029],checks=0;
 ...
 for(const name of ['shaderSource','attachShader','useProgram','bindFramebuffer','drawBuffers','drawElements','drawElementsInstanced','drawArrays','drawArraysInstanced'])native[name]=proto[name];
 ...
 for(const name of ['drawElements','drawElementsInstanced','drawArrays','drawArraysInstanced'])proto[name]=function(...args){
  const mask=framebuffer?fbMasks.get(framebuffer)||[this.COLOR_ATTACHMENT0]:backMask,key=`${id(program)}:${id(framebuffer)}:${mask.join(',')}`;
  ...
  if(before||error){const record={time:performance.now(),before,error,method:name,args,program:id(program),framebuffer:id(framebuffer),drawBuffers:mask,sources:(attached.get(program)||[]).map(s=>sources.get(s)),stack:new Error().stack};window.__cityGpuDiagnostics.errors.push(record);console.error('CITY_GPU_DIAGNOSTIC '+JSON.stringify({...record,sources:record.sources.map(s=>s?.slice(0,400))}));}
  return result;
 };
}
