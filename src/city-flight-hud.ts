...
export function createFlightHud(host:HTMLElement,world:DrivingWorld){
 ...
 <div class="flight-help" hidden><strong>W / S · 抬头 / 俯冲　 A / D · 左右侧倾</strong><span>方向键亦可操纵 · Q / E 方向舵 · Shift 加速 · 空格减速</span><span><button type="button" data-flight-mode="drone"><kbd>B</kbd> 无人机</button> · G / F 返回驾驶 · L 切换光照</span></div>`;
 ...
 return {update(menuOpen:boolean){
  ...
  if(s&&active){value('speed',Math.round(s.speed*3.6)+'<small>KM/H</small>');value('altitude',Math.round(s.altitude)+'<small>M</small>');value('heading',String(Math.round((s.yaw*180/Math.PI%360+360)%360)).padStart(3,'0')+'°');value('throttle',Math.round(s.throttle*100)+'<small>%</small>');(panel.querySelector('.flight-throttle i') as HTMLElement).style.width=s.throttle*100+'%';phase.textContent=s.phase==='exploding'?'飞行中断':'辅助平飞 · 松开按键自动回正';if(!crash.hidden)crash.querySelector('span')!.textContent=Math.max(1,Math.ceil((FLIGHT_RECOVERY_MS-(performance.now()-s.crashedAt))/1000))+' 秒后返回无人机';}
