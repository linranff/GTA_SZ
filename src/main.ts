...
function initUI(){
 ...
 world.onMessage=toast;world.onTick=dt=>{
  career?.tick(dt);
  $('#observer-help').hidden=!world.observer.active;$('#minimap').classList.toggle('observer-muted',world.observer.active);$('#speedometer').classList.toggle('observer-muted',world.observer.active||!!world.walk?.active);
  ...
  updateCinematicHud({speed:s.speed,distance:s.distance,roadName:world.roadName,district:nearest.area,night:world.night,lightMode:world.lightMode,walking:!!world.walk?.active,observer:world.observer.active,menuOpen:mapOpen||journalOpen||(world.paused&&!world.observer.active),navigation:!!selected});
  if(activeRide){const r=rides.find(r=>r.id===activeRide!.id)!,p=activeRide.phase==='pickup'?r.from:r.to,d=Math.hypot(s.x-p[0],s.z-p[1]);$('#ride-prompt').hidden=d>35;$('#ride-prompt').textContent=Math.abs(s.speed)>1?'靠边停车，与 '+r.person.split(' · ')[0]+' 交谈':'按 E · '+(activeRide.phase==='pickup'?'接上':'送达')+r.person.split(' · ')[0];}else {const prompt=career?.prompt()??(!career?.game.active&&world.walk?.active&&Math.hypot(world.actor.x-(career?.game.places.hub[0]??1e9),world.actor.z-(career?.game.places.hub[1]??1e9))<40?'按 E · 驿站接活 / 热饭补给':null);$('#ride-prompt').hidden=!prompt||mapOpen||journalOpen;$('#ride-prompt').textContent=prompt??'';}
