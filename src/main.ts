...
ui.innerHTML=`<div id="loading"><div class="brand">深城纪<span>SHENZHEN • OPEN ROADS</span></div><p>把下班后的时间，还给这座城市。</p><div class="load-line"><i></i></div><small id="progress">正在启动城市</small></div>`;
const canvas=document.querySelector<HTMLCanvasElement>('#game')!;
let cityMap:CityMapController|null=null;let world:DrivingWorld,graph:RoadGraph,selected:Landmark|null=null,route:V2[]=[],mapOpen=false,toastTime=0,hudTick=0,visited=new Set<string>();
...
function mapDraw(canvas:HTMLCanvasElement,_large=false){const o=world.observer;drawCinematicMinimap(canvas,world.data,{x:o.active?o.focus.x:world.state.x,z:o.active?o.focus.z:world.state.z,yaw:o.active?o.yaw:world.state.yaw,speed:world.state.speed,observer:o.active,route});}
...
function showJournal(open:boolean){if(open&&world.aerial)world.toggleAerial();if(open&&mapOpen)openMap(false);journalOpen=open;$('#life-journal').hidden=!open;world.paused=open;world.keys.clear();if(open)renderJournal();}
...
function interactRide(){if(!activeRide||world.paused)return;const r=rides.find(r=>r.id===activeRide!.id)!,p=activeRide.phase==='pickup'?r.from:r.to;const at=Math.hypot(world.state.x-p[0],world.state.z-p[1])<28;if(!at||Math.abs(world.state.speed)>1)return;
 ...
 if(settleRide(life,activeRide,r,world.state.distance,true,world.state.speed)){try{localStorage.setItem('shenchengji-city-life-v1',JSON.stringify(life));}catch{toast('浏览器未允许保存进度');}toast(r.arrivalLine+'  ·  收入 +¥ '+r.reward);toastTime=12;activeRide=null;selected=null;route=[];$('#route-hud').hidden=true;renderJournal();}
...
function initUI(){
 ...
 <div class="intro"><span>第一章 / 一路向海</span><h1>今晚，开远一点。</h1><p>南山 → 福田 → 罗湖，整座城市等你出发。</p></div>
 <div id="toast" role="status"></div><div id="route-hud" hidden><small>正在前往</small><strong id="route-title"></strong><span id="route-distance"></span><button id="cancel-autodrive" hidden>接管驾驶</button></div>
 ...
 <div class="controls"><span><kbd>W S</kbd> 油门 / 刹车</span><span><kbd>A D</kbd> 转向</span><span><kbd>空格</kbd> 手刹</span><span><kbd>C</kbd> 镜头</span><span><kbd>L</kbd> 日落 / 夜色</span><span><kbd>V</kbd> 看车</span><span><kbd>G</kbd> 无人机</span><span><kbd>R</kbd> 回到道路</span></div>
 <button id="journal-button">城市生活 <span id="wallet"></span> <kbd>J</kbd></button><div id="ride-prompt" hidden></div><section id="life-journal" hidden><div class="life-heading"><small>下班以后 / 城市手账</small><h2>今天，也为自己活一点。</h2><p id="life-savings"></p><progress id="life-progress" max="1800" value="180"></progress></div><div id="ride-list"></div><p class="life-note">这三位是虚构人物。接人和送达时停车，按 E 交谈。按 J 返回。</p></section><div id="observer-help" hidden><strong>无人机观景</strong><span>WASD 平移 · 方向键转头 · QE 升降 · Shift 加速</span><span>拖动环绕 · Shift + 拖动平移 · 滚轮远近 · G / F 返回驾驶</span></div><div id="fps" hidden></div><div id="credits">© OpenStreetMap contributors · ODbL　/　<a href="/licenses/open-city-assets.md" target="_blank" rel="noopener">环境素材 · CC0</a>　/　<a href="/licenses/carconcept-CC-BY-4.0.md" target="_blank" rel="noopener">CarConcept · DGG / Eric Chadwick · CC BY 4.0</a>　/　<a href="/licenses/coastal-terrain.md" target="_blank" rel="noopener">地形 · Copernicus</a></div>
 ...
 world.onMessage=toast;world.onTick=dt=>{
  $('#observer-help').hidden=!world.observer.active;$('#minimap').classList.toggle('observer-muted',world.observer.active);$('#speedometer').classList.toggle('observer-muted',world.observer.active);
  hudTick+=dt;toastTime-=dt;if(toastTime<=0)$('#toast').classList.remove('visible');if(hudTick<.12)return;hudTick=0;
  const s=world.state;
  const nearest=world.data.landmarks.reduce((a,b)=>Math.hypot(a.x-s.x,a.z-s.z)<Math.hypot(b.x-s.x,b.z-s.z)?a:b);
  updateCinematicHud({speed:s.speed,distance:s.distance,roadName:world.roadName,district:nearest.area,night:world.night,observer:world.observer.active,menuOpen:mapOpen||journalOpen||(world.paused&&!world.observer.active),navigation:!!selected});
  if(activeRide){const r=rides.find(r=>r.id===activeRide!.id)!,p=activeRide.phase==='pickup'?r.from:r.to,d=Math.hypot(s.x-p[0],s.z-p[1]);$('#ride-prompt').hidden=d>35;$('#ride-prompt').textContent=Math.abs(s.speed)>1?'靠边停车，与 '+r.person.split(' · ')[0]+' 交谈':'按 E · '+(activeRide.phase==='pickup'?'接上':'送达')+r.person.split(' · ')[0];}else $('#ride-prompt').hidden=true;
...
async function boot(){try{world=new DrivingWorld(canvas);await world.init(s=>{const p=document.querySelector('#progress');if(p)p.textContent=s;});graph=new RoadGraph(world.data.roads,await(await fetch('/city/navigation.json')).json());world.startTraffic(graph);initUI();initLife();Object.defineProperty(window,'__SHENCHENGJI_CITY__',{value:{get ready(){return world.ready;},get telemetry(){return {state:{...world.state},autopilot:world.autopilot?.status,render:world.diagnostics()};},get state(){return {...world.state,road:world.roadName,height:world.groundHeight(world.state.x,world.state.z),carY:world.car.position.y};},get performance(){return world.performance();},get stats(){return {coastal:world.coastal?.stats,bayWater:world.bayWater?.stats(),publicLighting:world.publicLighting?.stats(),buildingSigns:world.buildingSigns?.stats(),vehicleMaterials:world.vehicleMaterials?.stats,cockpit:world.cockpit?.stats,audio:world.audio.stats,tailLights:world.tailLights?.stats,autopilot:world.autopilot?.status,streetFurniture:world.streetFurniture?.stats,camera:{view:world.view,minZ:world.camera.minZ,position:world.camera.position.asArray(),fov:world.camera.fov},cinematic:world.cinematic?.stats,roadSurface:world.roadSurface?.stats,rainPuddles:world.rainPuddles?.stats,groundRelief:world.groundRelief?.stats,vehicleFinish:world.vehicleFinish?.stats,observer:world.observer.status,architecture:world.architecture.stats(),facadeDiversity:world.facadeDiversity.stats(),landscape:world.landscape?.stats,signage:world.signage?.stats(),life:{cash:life.cash,completed:[...life.completed],active:activeRide?{...activeRide}:null,rides:rides.map(r=>({...r}))},lighting:{carPaint:world.carPaintDiagnostics(),environmentReady:!!world.scene.environmentTexture?.sphericalPolynomial,treeInstances:world.landscape?.stats.sourceTreeCount,carMaterial:world.carMeshes[0]?.material?.name,headlightsExcludeWater:world.headlights.every(l=>l.excludedMeshes.some(m=>m.name==='terrain_water'))},counts:world.data.meta.counts,landmarks:world.data.landmarks,graphNodes:graph.nodes.length,pedestrians:world.pedestrians?.people.map(p=>({x:p.x,z:p.z})),trafficMeshes:world.traffic?.meshes.flat().map(m=>({name:m.name,parent:m.parent?.name,instances:m.thinInstanceCount,enabled:m.isEnabled()})),traffic:world.traffic?.cars.map(c=>({x:c.x,z:c.z})),photo:world.photoTarget?.id};},resetPerformance(){world.samples=[];}}});}catch(e){console.error(e);ui.innerHTML=`<div id="loading"><h1>城市暂时没有载入</h1><p>${e instanceof Error?e.message:'请刷新后重试'}</p><button onclick="location.reload()">重新加载</button></div>`;}}
void boot();
