...
export class CityTraffic{
 ...
 /** Distance to a red stop line ahead of (x,z,yaw), or null; cars hold within 6 m of it. */
 ...
 place(x:number,z:number){this.cars=[];const candidates:number[]=[];for(let i=0;i<this.graph.nodes.length;i++){const p=this.graph.nodes[i],d=Math.hypot(p[0]-x,p[1]-z);if(d>14&&d<600&&this.graph.edges[i].size>1)candidates.push(i);}
