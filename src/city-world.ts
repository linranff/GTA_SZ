...
import {CityCollision,stepCar,clamp,type CarState} from './driving.ts';
export class DrivingWorld{
 ...
 state:CarState={x:0,z:0,yaw:0,speed:0,steer:0,distance:0};ready=false;paused=false;night=false;view=0;keys=new Set<string>();
 ...
 constructor(public canvas:HTMLCanvasElement){
  ...
  this.camera=new FreeCamera('driving-chase',new Vector3(0,4,-8),this.scene);this.camera.minZ=.75;this.camera.maxZ=9000;this.camera.fov=.9;this.camera.inputs.clear();
  ...
  this.waterMirror=new MirrorTexture('bay-reflection',512,this.scene,true);this.waterMirror.mirrorPlane=new Plane(0,-1,0,-.12);this.waterMirror.refreshRate=3;this.waterMirror.blurKernel=5;this.waterMirror.level=.68;
  ...
  canvas.addEventListener('pointermove',e=>{if(!this.drag||e.pointerId!==dragPointer||!e.buttons)return;const dx=e.clientX-this.pointer[0],dy=e.clientY-this.pointer[1];if(this.observer.active){if(e.shiftKey)this.observer.pan(dx,dy,canvas.clientHeight);else this.observer.rotate(dx,dy);}else{this.cameraYaw+=dx*.005;this.cameraPitch=clamp(this.cameraPitch+dy*.002,.02,.8);}this.pointer=[e.clientX,e.clientY];});
  ...
  canvas.addEventListener('touchmove',e=>{if(!touchCentroid||!e.touches.length)return;e.preventDefault();const next=touchPosition(e.touches),dx=next.x-touchCentroid.x,dy=next.y-touchCentroid.y;if(this.observer.active){if(e.shiftKey)this.observer.pan(dx,dy,canvas.clientHeight);else this.observer.rotate(dx,dy);}else{this.cameraYaw+=dx*.005;this.cameraPitch=clamp(this.cameraPitch+dy*.002,.02,.8);}touchCentroid=next;},{passive:false});
 ...
 async init(progress:(s:string)=>void){
  progress('正在展开深圳地图');const response=await fetch('/city/city.json');if(!response.ok)throw Error('地图加载失败');this.data=await response.json();const detail=await loadLandmarkDetails(this.data);if(detail){this.groundHeight=detail.heightAt;this.detailManifest=detail.manifest;}progress('正在铺设公园缓坡');this.groundRelief=await loadCityGroundRelief(this.groundHeight);this.groundHeight=this.groundRelief.heightAt;this.collision=new CityCollision(this.data);this.windowSources=[];this.state={...this.data.spawn,speed:0,steer:0,distance:0};
  progress('正在铺设海岸线和城市道路');await this.load('terrain');const roads=await this.load('roads');
  ...
  progress('正在装配深圳地标');const lm=await this.load('landmarks');this.landmarks=lm.meshes.filter(m=>m.getTotalVertices()>0);if(this.detailManifest){const prefixes=this.detailManifest.replacedMeshPrefixes;for(const m of [...this.scene.meshes])if(prefixes.some(p=>m.name.startsWith(p)))m.dispose(false,false);this.blocks=this.blocks.filter(b=>!b.mesh.isDisposed());this.landmarks=this.landmarks.filter(m=>!m.isDisposed());const details=await this.load('landmark-detail');for(const mesh of details.meshes.filter(m=>m.getTotalVertices()>0)){const tile=mesh.name.match(/detail_block_(-?\d+)_(-?\d+)_/);if(tile)this.blocks.push({mesh,x:(Number(tile[1])+.5)*640,z:(Number(tile[2])+.5)*640,road:false});else this.landmarks.push(mesh);}}
 ...
 startAutoDrive(destination:Landmark){if(!this.autopilot)return;if(this.observer.active)this.exitPhoto();this.keys.clear();this.paused=false;this.autopilot.start(destination);this.previousAutoPhase='driving';this.audio.cue('engage');this.onMessage?.('自动驾驶 · '+destination.name+' · 方向键 / WASD 随时接管');}
 ...
 sceneFocus(){return this.observer.active?this.observer.focus:this.state;}
 ...
 aerialEffects(active:boolean){if(this.overviewEffects===active)return;this.overviewEffects=active;this.sun.orthoLeft=this.sun.orthoBottom=active?-1050:-260;this.sun.orthoRight=this.sun.orthoTop=active?1050:260;this.sun.shadowMaxZ=active?3200:1200;this.shadows.getShadowMap()!.refreshRate=active?12:1;this.shadows.getShadowMap()!.resetRefreshCounter();this.mirror.refreshRate=active?6:2;this.waterMirror.refreshRate=active?6:3;if(this.ao){if(active)this.scene.postProcessRenderPipelineManager.detachCamerasFromRenderPipeline('contact-shading',this.camera);else this.scene.postProcessRenderPipelineManager.attachCamerasToRenderPipeline('contact-shading',this.camera);}this.cull();}
 toggleAerial(){if(this.observer.active){this.exitPhoto();return;}const s=this.state;this.enterPhoto({id:'free-camera',name:'自由无人机',x:s.x,z:s.z,height:0,area:'城市全景',excludeRadius:0,arrival:[s.x,s.z],yaw:s.yaw,photoDistance:420,photoElevation:.42,photoAngle:.65,photoTargetHeight:this.groundHeight(s.x,s.z)+15});}
 ...
 setupReflections(){
  ...
  const water=new StandardMaterial('living-bay',this.scene);water.maxSimultaneousLights=2;water.disableLighting=true;water.emissiveColor=new Color3(.015,.06,.07);water.diffuseColor=new Color3(.025,.17,.19);water.specularColor=new Color3(.92,.78,.54);water.specularPower=140;water.reflectionTexture=this.waterMirror;water.reflectionFresnelParameters=new FresnelParameters();water.reflectionFresnelParameters.bias=.1;water.reflectionFresnelParameters.power=3;water.reflectionFresnelParameters.leftColor=Color3.White();water.reflectionFresnelParameters.rightColor=new Color3(.25,.25,.25);
  const n=128,pixels=new Uint8Array(n*n*3);for(let y=0;y<n;y++)for(let x=0;x<n;x++){const k=(y*n+x)*3;pixels[k]=128+18*Math.sin(x*.3+y*.4);pixels[k+1]=128+12*Math.cos(y*.25+x*.12);pixels[k+2]=250;}
  const normal=RawTexture.CreateRGBTexture(pixels,n,n,this.scene,true,false);normal.uScale=750;normal.vScale=750;normal.level=.22;water.bumpTexture=normal;
  const waterMeshes=this.scene.meshes.filter(m=>m.name==='terrain_water');for(const mesh of waterMeshes)mesh.material=water;for(const light of this.headlights)light.excludedMeshes.push(...waterMeshes);
 ...
 setupSigns(){
  ...
  let count=0;for(const road of this.data.roads){if(road.points.length<4||!['primary','trunk'].includes(road.kind)||road.name.includes('辅'))continue;const a=road.points[0],b=road.points[1];if(Math.abs(a[0]-this.data.spawn.x)>650||Math.abs(a[1]-this.data.spawn.z)>700)continue;if(count++>8)break;
   const sign=MeshBuilder.CreatePlane('direction-sign',{width:8,height:2,sideOrientation:Mesh.DOUBLESIDE},this.scene);const mat=new StandardMaterial('wayfinding',this.scene);mat.diffuseTexture=new Texture('/city/textures/sign-'+(b[0]<a[0]?'west':'east')+'.jpg',this.scene);(mat.diffuseTexture as Texture).anisotropicFilteringLevel=8;mat.specularColor=Color3.Black();mat.emissiveColor=new Color3(.05,.05,.04);sign.material=mat;sign.position.set(a[0],6.8,a[1]);sign.rotation.y=Math.atan2(b[0]-a[0],b[1]-a[1])+Math.PI;sign.isPickable=false;
  }
 }
 ...
 localLights(dt:number){
  ...
  if(this.lightTick<=0){this.lightTick=.3;let first:number[]|null=null,second:number[]|null=null,d1=Infinity,d2=Infinity;for(const l of this.lampData){const d=(l[0]-p.x)**2+(l[1]-p.z)**2;if(d<d1){second=first;d2=d1;first=l;d1=d;}else if(d<d2){second=l;d2=d;}}
   this.lightTargets=[first,second].map((l,i)=>({position:l?new Vector3(l[0],this.groundHeight(l[0],l[1])+8,l[1]):Vector3.Zero(),power:(i?d2:d1)<4900?(this.night?140:70):0}));
  }
  this.streetLights.forEach((l,i)=>{const t=this.lightTargets[i];if(!t)return;const changing=Vector3.DistanceSquared(l.position,t.position)>1;const target=changing?0:t.power;l.intensity+=(target-l.intensity)*(1-Math.exp(-dt*12));if(changing&&l.intensity<.8)l.position.copyFrom(t.position);});
 }
 ...
 toggleLight(){this.night=!this.night;this.skyMat.setFloat('night',this.night?1:0);this.sun.intensity=this.night?.13:3.0;this.hemi.intensity=this.night?.50:.88;this.scene.environmentIntensity=this.night?.40:.85;this.scene.fogColor=this.night?new Color3(.085,.14,.22):new Color3(.38,.24,.30);this.onMessage?.(this.night?'蓝调夜色 · 车灯已开启':'海湾日落');this.architecture.setNight(this.night);this.facadeDiversity.setNight(this.night);this.signage?.setNight(this.night);this.buildingSigns?.setNight(this.night);this.cinematic?.setNight(this.night);}
 resetRoad(){if(!this.ready)return;this.cancelAutoDrive('reset-road');const n=this.collision.nearest(this.state.x,this.state.z);if(n){this.state.x=n.x;this.state.z=n.z;this.state.yaw=n.yaw;this.state.speed=0;this.state.steer=0;this.cameraYaw=n.yaw;this.onMessage?.('已回到 '+n.road.name);}else this.travel(this.data.landmarks.find(m=>m.id==='baypark')!);}
 travel(m:Landmark){this.cancelAutoDrive('debug-travel');if(this.observer.active)this.exitPhoto();this.state.x=m.arrival[0];this.state.z=m.arrival[1];this.state.yaw=m.yaw;this.state.speed=0;this.state.steer=0;this.cameraYaw=m.yaw;this.car.position.set(this.state.x,this.groundHeight(this.state.x,this.state.z)+.115,this.state.z);this.camera.position.set(this.state.x-Math.sin(m.yaw)*9,this.groundHeight(this.state.x,this.state.z)+4,this.state.z-Math.cos(m.yaw)*9);this.cull();this.vegetation();this.traffic?.place(this.state.x,this.state.z);this.pedestrians?.place(this.state.x,this.state.z);this.onMessage?.('已抵达 '+m.name+' 周边道路');}
 update(dt:number){
  this.time+=dt;this.localLights(dt);const oldCarX=this.car.position.x,oldCarZ=this.car.position.z;if(!this.paused){const n=this.collision.nearest(this.state.x,this.state.z);this.offroad=!n||n.d>n.road.width/2+1;if(n&&n.d<40)this.roadName=n.road.name;
   ...
   const manual={throttle,steer,handbrake:this.keys.has('Space')};const status=this.autopilot?.status;
  ...
  this.cockpit?.setActive(this.view===1&&!this.observer.active);this.cockpit?.update(s.speed,s.steer/.48,{night:this.night,distanceMetres:s.distance});
  ...
  const mat=this.scene.getMaterialByName('living-bay') as StandardMaterial;if(mat?.bumpTexture){(mat.bumpTexture as Texture).uOffset=this.time*.0014;(mat.bumpTexture as Texture).vOffset=this.time*.0005;}
  this.onTick?.(dt);
 }
