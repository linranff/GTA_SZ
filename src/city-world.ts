...
export class DrivingWorld{
 ...
 async init(progress:(s:string)=>void){
  ...
  progress('正在种植榕树、棕榈与花境');this.landscape=new CityLandscape(this.scene,this.groundHeight,(x,z,r)=>!!this.bambooCafe?.layout.reserved(x,z,r));await this.landscape.init();progress('正在栽种榕树、木棉与樟树林冠');this.canopy=new CityCanopy(this.scene,this.groundHeight,(x,z,r)=>!!this.bambooCafe?.layout.reserved(x,z,r));await this.canopy.init();
