...
import {loadLandmarkDetails,loadLandmarkCandidates,type LandmarkDetailManifest,type LandmarkCandidateManifest} from './landmark-details.ts';
...
import {CityEbikes} from './city-ebikes.ts';
...
export class DrivingWorld{
 ...
 async init(progress:(s:string)=>void){
  ...
  if(this.candidateManifest){progress('正在替换候选地标模型');const prefixes=this.candidateManifest.replacedMeshPrefixes;for(const m of [...this.scene.meshes])if(prefixes.some(p=>m.name.startsWith(p)))m.dispose(false,false);this.landmarks=this.landmarks.filter(m=>!m.isDisposed());const candidates=await this.load('landmark-candidates');this.landmarks.push(...candidates.meshes.filter(m=>m.getTotalVertices()>0));}
  ...
  this.bambooCorridor.adoptParkedCars(this.trafficSources);
 ...
 setLightMode(mode:'sunset'|'night'|'day'){
  ...
  this.architecture.setMode(mode);this.facadeDiversity.setMode(mode);this.signage?.setMode(mode);this.buildingSigns?.setNight(this.night);this.signals?.setNight(this.night);
