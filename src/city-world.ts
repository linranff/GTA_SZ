...
export class DrivingWorld{
 ...
 constructor(public canvas:HTMLCanvasElement){
  ...
  window.addEventListener('keydown',e=>{if(e.target instanceof HTMLElement&&e.target.closest('input,textarea,select,[contenteditable=true]'))return;if(['KeyW','KeyS','KeyA','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','Tab'].includes(e.code))e.preventDefault();this.keys.add(e.code);if(!this.paused&&['KeyW','KeyS','KeyA','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(e.code))this.cancelAutoDrive('manual-takeover');if(e.repeat)return;if(e.code==='KeyB')void this.toggleFlight();if(e.code==='KeyT')void this.toggleTank();if((e.code==='Enter'||e.code==='Space')&&this.tank?.active&&!this.paused&&!this.walk?.active)this.tank.fire();if(e.code==='KeyH')this.audio.cue('horn');if(e.code==='KeyG')this.toggleAerial();if(e.code==='KeyR'&&!this.observer.active)this.resetRoad();if(e.code==='KeyC'&&!this.observer.active&&this.walk?.active){this.walkFirstPerson=!this.walkFirstPerson;this.onMessage?.(this.walkFirstPerson?'步行 · 第一人称':'步行 · 第三人称');}else if(e.code==='KeyC'&&!this.observer.active){this.view=(this.view+1)%3;this.onMessage?.(['追踪镜头','驾驶舱镜头','远景镜头'][this.view]);}if(e.code==='KeyL')this.toggleLight();if(e.code==='KeyV'){if(this.observer.active)this.exitPhoto();else this.enterPhoto({id:'car',name:'绯红海湾 GT',x:this.state.x,z:this.state.z,height:1.45,area:'车辆',excludeRadius:0,arrival:[this.state.x,this.state.z],yaw:this.state.yaw,photoDistance:7.7,photoElevation:.18,photoAngle:this.state.yaw+Math.PI+.65,photoTargetHeight:this.groundHeight(this.state.x,this.state.z)+.65});}if(e.code==='KeyF'){if(this.observer.active)this.exitPhoto();else if(!this.paused)void this.toggleWalking();}});
 ...
 async toggleTank(){
  ...
  this.tank.setPose(this.state.x,this.groundHeight(this.state.x,this.state.z),this.state.z,this.state.yaw);
 ...
 async toggleFlight(){
  ...
  try{await flight.load();if(request!==this.flightRequest||!this.observer.active||this.scene.isDisposed)return;
   ...
   this.paused=true;this.cull();this.onMessage?.('湾翼 · W/S 抬头俯冲 · A/D 侧倾 · Shift 加速 · B 返回无人机');
 ...
 update(dt:number){
  ...
  if(!this.paused&&!this.walk?.active){const n=this.collision.nearest(this.state.x,this.state.z);this.offroad=!n||n.d>n.road.width/2+1;if(n&&n.d<40)this.roadName=roadDisplayName(n.road);
   ...
   const impact=this.pedestrians?.collideVehicle({...before,groundY:this.groundHeight(before.x,before.z)},{...this.state,groundY:this.groundHeight(this.state.x,this.state.z)});
