...
export class DrivingWorld{
 ...
 async init(progress:(s:string)=>void){
  progress('正在展开深圳地图');const response=await fetch('/city/city.json');if(!response.ok)throw Error('地图加载失败');this.data=await response.json();const detail=await loadLandmarkDetails(this.data);if(detail){this.groundHeight=detail.heightAt;this.detailManifest=detail.manifest;}progress('正在铺设公园缓坡');this.groundRelief=await loadCityGroundRelief(this.groundHeight);this.groundHeight=this.groundRelief.heightAt;progress('正在架设跨水桥梁');this.coastal=await loadCoastalInfrastructure(this.data,this.groundHeight);this.groundHeight=this.coastal.heightAt;this.collision=new CityCollision(this.data);this.windowSources=[];this.state={...this.data.spawn,speed:0,steer:0,distance:0};
 ...
 localLights(dt:number){
  ...
  if(this.lightTick<=0){this.lightTick=.25;
   ...
   this.lightTargets=this.lampAssignments.map(id=>{const l=this.lampData[id],d=l?Math.hypot(l[0]-p.x,l[1]-p.z):Infinity;return {position:l?new Vector3(l[0]-l[2]*1.5,this.groundHeight(l[0],l[1])+8.2,l[1]-l[3]*1.5):Vector3.Zero(),power:(this.night?360:75)*clamp((108-d)/30,0,1)};});
  }
 ...
 toggleLight(){this.night=!this.night;this.skyMat.setFloat('night',this.night?1:0);this.sun.intensity=this.night?.13:3.0;this.hemi.intensity=this.night?.50:.88;this.scene.environmentIntensity=this.night?.40:.85;this.scene.fogColor=this.night?new Color3(.085,.14,.22):new Color3(.38,.24,.30);this.onMessage?.(this.night?'蓝调夜色 · 车灯已开启':'海湾日落');this.architecture.setNight(this.night);this.facadeDiversity.setNight(this.night);this.signage?.setNight(this.night);this.buildingSigns?.setNight(this.night);this.cinematic?.setNight(this.night);this.bayWater?.setNight(this.night);this.publicLighting?.setNight(this.night);}
