...
| 上空から見ると芝生や公園が裸に見える | 独立した 10–30 m の樹冠レイヤー：ガジュマル・キワタ・クスノキ・モモタマナ・ホウオウボク・マンゴーの 6 種（不透明な塊状の樹冠）、45,626 本を公園・空き地の芝生・幹線道路沿いに配置し、間隔をオフラインで検証。3 段階 LOD、予算超過は距離で降格。地表の草の予算は半減 | 約 1–5 fps のコスト（主に影）。樹種は美術的な習作で植生調査ではない。[記録](docs/graphics/canopy-trees-2026-09-17.md) |
| 交差点に信号がなく、街に電動バイクがない | OSM 幹線道路の共有ノードから 395 の信号交差点を集約し、片持ち式信号機を交差車線の先の停止線・運転者右側または中央分離帯に設置、2 系統固定サイクル。NPC 車両・ライダー・自動運転は赤で停止線に待機。21,110 台の電動スクーター／電動自転車／配達バイクを地下鉄出口・バス停・密集街区の路肩に列駐車し、インスタンスごとに着色。ヘルメット姿のライダー少数が路肩を走行 | 信号位置と駐車列はゲーム上の翻案で実測ではない。ライダーはまだ車や歩行者を避けない。[信号](src/city-traffic-signals.ts) · [電動バイク](src/city-ebikes.ts) |
